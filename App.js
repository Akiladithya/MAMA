...
export default function App() {
  ...
  const generateMCQs = async () => {
    if (!highlightText) return alert("Enter some text to generate MCQs.");
    ...
    try {
      const res = await fetch('http://127.0.0.1:5000/generate-mcqs', {
        ...
        body: JSON.stringify({ text: highlightText })
      });
      ...
      alert("Failed to generate MCQs. Make sure the backend is running.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.buttonGroup}>
              ...
              <Button mode="contained" disabled={!highlightText} onPress={generateMCQs}>
                Generate MCQs
              </Button>
            </View>
            ...
            <TextInput
              label="Enter text to highlight and generate MCQs"
              value={highlightText}
            ...
            {mcqs.length > 0 && (
              <View style={{ marginTop: 20 }}>
                <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 10 }}>🧠 Generated MCQs:</Text>
                {mcqs.map((mcq, index) => (
                  <View key={index} style={{ marginBottom: 20 }}>
                    <Text style={{ fontSize: 16 }}>{index + 1}. {mcq.question}</Text>
                    {mcq.options.map((option, i) => (
                      <Text key={i} style={{ marginLeft: 10 }}>• {option}</Text>
                    ))}
                    <Text style={{ marginLeft: 10, fontStyle: 'italic', marginTop: 5 }}>Answer: {mcq.answer}</Text>
                  </View>
                ))}
              </View>
            )}
